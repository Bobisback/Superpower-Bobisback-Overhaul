...
--	end	
	...
	local SFAirTarget1ID = GameInfo.UnitPromotions["PROMOTION_SP_FORCE_AIR_ATTACK_1"].ID
	local SFAirTarget2ID = GameInfo.UnitPromotions["PROMOTION_SP_FORCE_AIR_ATTACK_2"].ID
	local SFAirTarget3ID = GameInfo.UnitPromotions["PROMOTION_SP_FORCE_AIR_ATTACK_3"].ID	
...
--					iCounterLv1=2
	...
	if pAttackingUnit:IsHasPromotion(SFAirTarget1ID) then ------------------------Spical Forces attack air units inside the city
		if pDefendedPlot then
			 if pDefendedPlot:IsCity() then			 				
				pDefendingCity = pDefendedPlot:GetPlotCity()
				print("Attak AirCraft attacking City!")
				...
				if unitCount > 0 then 
					...
					for i = 0, unitCount-1, 1 do
						local pFoundUnit = pDefendedPlot:GetUnit(i)	
						if (pFoundUnit ~= nil) and (pFoundUnit:GetUnitCombatType()== GameInfoTypes.UNITCOMBAT_FIGHTER or pFoundUnit:GetUnitCombatType()== GameInfoTypes.UNITCOMBAT_ARCHER) then --make sure this is a aircraft
							pFoundUnit:ChangeDamage(20)
							print ("Air units in the city are attacked!")
							if pAttackingUnit:IsHasPromotion(SFAirTarget2ID) then 
								pFoundUnit:ChangeDamage(10)
							end	
							if pAttackingUnit:IsHasPromotion(SFAirTarget3ID) then 
								pFoundUnit:ChangeDamage(10)						
							end
						end
					end
...
--*******************************************************************************Unit City Retreat Rules*******************************************************************************

--------------------------------------------------------------
-- Globals
--------------------------------------------------------------
MIN_HP_LEFT_BEFORE_ESCAPING_CITY	= 20 -- under that value of predicted HP left to a city after a combat that is starting, units will try to escape a city
			...
			if unit:GetDomainType() == DomainTypes.DOMAIN_AIR then
				...
				for i, escapePlot in pairs(escapeList) do					
					...
					if (escapePlot:GetArea() == plot:GetArea() or escapePlot:Area():GetNumTiles() > 15) -- same area or not a lake/island
					   ...
					   --and AreSameSide( escapePlot:GetOwner(), unit:GetOwner()) --I do not think we need this if we do i will write the function for it
					   then 						
