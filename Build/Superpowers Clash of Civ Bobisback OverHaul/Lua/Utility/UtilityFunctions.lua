	...
	if player:GetNumCities() > 0 then
		...
		elseif city:IsPuppet() then
			...
			if player:GetCivilizationType() == GameInfoTypes["CIVILIZATION_VENICE"] then
				...
				city:SetNumRealBuilding(GameInfoTypes["BUILDING_PUPPET_GOVERNEMENT"],0)
--				local iRand = Game.Rand(10, "Random number for puppet cities");
--				if iRand == 0 then
--					city:SetFocusType(2)
--                elseif iRand == 1 then
					city:SetFocusType(5)
...
function GetCloseNavalCity ( playerID, plot , bNotSelf)
	...
	if pPlayer then
		...
		for pCity in pPlayer:Cities() do
			if pCity:Plot() == plot and bNotSelf then
				Dprint ( " -- GetCloseCity() called with (bNotSelf = true), don't test city on starting plot...", bDebug)
			elseif not pCity:IsCoastal() then
				Dprint ( " --  " .. pCity:GetName() .. " is not a coastal city...", bDebug)
			else
				distanceToCity = Map.PlotDistance(pCity:GetX(), pCity:GetY(), plot:GetX(), plot:GetY())
				if ( distanceToCity < distance) then
					distance = distanceToCity
...
function CanSharePlot(unit, plot)
	...
	for i = 0, numUnits do
		...
		if plotUnit ~= nil and plotUnit ~= unit and unit:GetOwner() ~= plotUnit:GetOwner() then
			return false
		end
	end
	return true
